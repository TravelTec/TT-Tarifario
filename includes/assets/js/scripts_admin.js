jQuery(document).ready(function($){ 
	...
	$('input[name=QtdPax]').mask('9');
	if($('input[name=QtdPax]').val() > 4){
		$('input[name=QtdPax]').val(4);
	}
...
function adicionar_novo_tarifario_tarifa(){
	console.log(jQuery("#typePeriod").val());
	//jQuery(".repeater_tarifario").attr("style", "padding: 11px 10px;");
    for (var i = 1; i < 50; i++) {
    ...
    .datepicker({
      ...
      dateFormat: 'dd/mm/yy',
        onSelect: function (selectedDate) {
            ...
            jQuery("#data_final"+iti_index).datepicker("option", "minDate", data);
        }
    });
    ...
    .datepicker({
      ...
      dateFormat: 'dd/mm/yy',
        onSelect: function (selectedDate) {  
			...
			var data_checkin = jQuery("#data_inicial"+iti_index).val();
    var data_checkout = jQuery("#data_final"+iti_index).val();

    var data = jQuery('#data_inicial'+iti_index).val()+' - '+jQuery('#data_final'+iti_index).val();
    data = data.split(" - ");
    ...
    const noites = (parseInt(days) == 1 ? parseInt(days) : parseInt(days)); 
			...
			if(noites > parseInt(jQuery("#dias").val()) || noites < parseInt(jQuery("#dias").val())){ 
				
				swal({
					title: "Data incorreta",
					text: 'Você precisa informar uma data que seja compatível com a quantidade de dias do pacote.',
					icon: "error",
				}); 
			} 
			
			var date2 = jQuery("#data_inicial"+iti_index).datepicker('getDate', '+'+parseInt(jQuery("#dias").val())+'d'); 
			  date2.setDate(date2.getDate()+parseInt(jQuery("#dias").val())); 
			  jQuery("#data_final"+iti_index).datepicker('setDate', date2); 
		}
    });
}
...
function show_dados_tarif(key){
    ...
    .datepicker({
      ...
      dateFormat: 'dd/mm/yy',
        onSelect: function (selectedDate) {
            //Limpamos a segunda data, para evitar problemas do usuário ficar trocando a data do primeiro datepicker e acabar burlando as regras definidas.
            jQuery.datepicker._clearDate(jQuery("#data_final"+key)); 
            //Aqui está a "jogada" para somar os 7 dias para o próximo datepicker.
            var data = jQuery.datepicker.parseDate('dd/mm/yy', selectedDate); 
            jQuery("#data_final"+key).datepicker("option", "minDate", data);
        }
    });
    jQuery('#data_final'+key)
    .datepicker({
      language: 'pt-BR', 
      autoClose: true,
      dateFormat: 'dd/mm/yy',
        onSelect: function (selectedDate) {  
			...
			var data_checkin = jQuery("#data_inicial"+key).val();
    var data_checkout = jQuery("#data_final"+key).val();

    var data = jQuery('#data_inicial'+key).val()+' - '+jQuery('#data_final'+key).val();
    data = data.split(" - ");
    var data_inicio = data[0].split("/");
    data_inicio = data_inicio[2]+"-"+data_inicio[1]+"-"+data_inicio[0];
    var data_fim = data[1].split("/");
    ...
    const noites = (parseInt(days) == 1 ? parseInt(days) : parseInt(days)); 
			...
			var date2 = jQuery("#data_inicial"+key).datepicker('getDate', '+'+parseInt(jQuery("#dias").val())+'d'); 
			  date2.setDate(date2.getDate()+parseInt(jQuery("#dias").val())); 
			  jQuery("#data_final"+key).datepicker('setDate', date2); 
		}
    });
}
    ...
    /* ******************* ACOMODAÇÕES ******************* */
        ...
        for (var i = 1; i < 50; i++) { 
            if(jQuery("#apto_roteiro"+i).val() != undefined){ 
                ...
                hotel_roteiro.push(jQuery("#hotel_roteiro"+i).val().replace("'", ""));
...
function select_apto_roteiro(key, type = null){
	var apto_roteiro = jQuery("#apto_roteiro"+key).val();
	var qtd_pax = jQuery("#qtd_pax_"+apto_roteiro).val();
	if(type == 1){
		jQuery.ajax({
			type: "POST",
			url: wp_ajax.ajaxurl,
			data: { action: "get_data_apto", apto_roteiro:apto_roteiro },
			success: function( data ) {
				console.log('data :'+data.slice(0,-1));
				qtd_pax = data.slice(0,-1);
				
				if(qtd_pax == 1){ 
		jQuery("#div_valor_pacote_single"+key).attr("style", "padding:10px");
		jQuery("#div_valor_pacote_double"+key).attr("style", "display:none");
		jQuery("#div_valor_pacote_triple"+key).attr("style", "display:none");
	}else if(qtd_pax == 2){
		jQuery("#div_valor_pacote_single"+key).attr("style", "display:none"); 
		jQuery("#div_valor_pacote_double"+key).attr("style", "padding:10px");
		jQuery("#div_valor_pacote_triple"+key).attr("style", "display:none"); 
	}else if(qtd_pax == 3){
		jQuery("#div_valor_pacote_single"+key).attr("style", "display:none");
		jQuery("#div_valor_pacote_double"+key).attr("style", "display:none"); 
		jQuery("#div_valor_pacote_triple"+key).attr("style", "padding:10px");
	}else{
		jQuery("#div_valor_pacote_single"+key).attr("style", "display:none"); 
		jQuery("#div_valor_pacote_double"+key).attr("style", "padding:10px");
		jQuery("#div_valor_pacote_triple"+key).attr("style", "display:none");
	...
	console.log('qtd_pax :'+qtd_pax);
	var room = jQuery("#room_"+apto_roteiro).val();
	jQuery("#divValorPacote"+key).attr("style", "padding-top:10px");
	if(qtd_pax == 1){ 
		jQuery("#div_valor_pacote_single"+key).attr("style", "padding:10px");
		jQuery("#div_valor_pacote_double"+key).attr("style", "display:none");
		jQuery("#div_valor_pacote_triple"+key).attr("style", "display:none");
	...
	jQuery("#div_valor_pacote_single"+key+" label").attr("style", "display:none");
	jQuery("#div_valor_pacote_double"+key+" label").attr("style", "display:none");
	jQuery("#div_valor_pacote_triple"+key+" label").attr("style", "display:none");
}
/* ************************************** */
